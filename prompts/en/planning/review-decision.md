...
- `${merged_review_output_path}` is a workflow runtime artifact and may only be used as revision guidance for the next execution iteration; do not ask execution to write the merged review, reviewer comments, adjudication memory, or adjudication process into the formal planning file in the project workspace.
- If execution's runtime artifact declares `[DELIVERABLE]` and a formal planning file path, subsequent revision items should primarily target the planning body in that formal file, not the runtime report in `${agent_output_path}`.
- Revision instructions for execution must keep the formal planning file as a clean, directly deliverable document. Do not ask execution to add workflow/review process sections such as "previous review result", "this round's revision notes", "accepted reviewer comments", "review response", "changelog", `MUST_FIX`, or `CLOSABLE_ACCEPTANCE_ITEMS`, and do not ask for any `0.`-prefixed workflow revision note.
- If a reviewer finds that the formal planning file already contains workflow/review process content, and the task did not explicitly ask to keep a changelog, prefer compressing "remove that process content and fold any necessary information into the planning body" into one closable acceptance item.
