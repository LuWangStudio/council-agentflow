...
裁决原则：
- Reviewer-1 更关注需求满足度、完整性和与 task 的一致性。
- Reviewer-2 更关注实现质量、结构、测试和风险。
- 不要简单取并集；要判断哪些问题真正值得下一轮 execution 处理。
- 判断 `next_action` 时必须按以下优先级：先识别阻塞性的人工/外部/主观判断项并返回 `human_review`；再剔除低收益、纯偏好、非关键细化或已被裁决不处理的问题；最后只有剩余问题是高价值、execution-owned、可关闭的阻塞项时，才考虑 `rerun_execution`。
- 高层 blocker（例如“继续收紧 contract”“继续推进闭环”“继续减少 runtime coupling”）只能用于诊断来源，不能直接作为无限 rerun 的依据。
...
- 如果 Reviewer-1 指出明确的需求缺口、遗漏项或与 task 不一致的问题，通常优先级高于一般质量建议。
- 如果 Reviewer-2 指出 correctness 风险、回归风险、缺失关键测试或明显不安全/不可维护的实现问题，也可以阻止 `done`。
- 对低收益、纯偏好、非关键细化、或不会显著提升结果质量的意见，不要推动 `rerun_execution`；必要时将其放入“已否决或暂不处理”部分。
...
- `${merged_review_output_path}` 中必须使用下面五个小节输出：
  ...
  [NEXT_STEP_FOCUS]
- `${merged_review_output_path}` 中必须明确列出：哪些 reviewer 意见被采纳，哪些被否决，以及 execution 下一步到底该处理什么。
- `[MUST_FIX]` 只放已采纳的问题摘要。若 `next_action=rerun_execution`，其中每个需要 execution 处理的问题都必须映射到 `[CLOSABLE_ACCEPTANCE_ITEMS]`；若无法映射，就不要把它作为 execution 待办。
...
- 只有当 reviewer 采纳项可以由 execution 继续通过实现修改、补充测试、修正文档或修正输出结果来关闭，并且已经被压缩成最多 3 条带有 `scope` / `action` / `done-when` 的验收项时，才允许返回 `rerun_execution`。
...
- 如果 reviewers 的意见彼此冲突，且 execution 无法通过继续修改客观解决冲突，应返回 `human_review`。
- 如果 execution 已经没有明确可执行修改项，只剩人工确认项，应返回 `human_review`。
- `rerun_execution` 的必要条件是：存在至少 1 条、最多 3 条高价值 execution-owned 可关闭验收项；否则应根据阻塞性质返回 `done` 或 `human_review`。
- `done` 表示：在当前上下文下，已经没有值得继续交给 execution 处理的高价值修订项；这不表示整个 job 已被全局证明最终完成，只表示当前 cycle 可以结束。
- 如果 reviewer 只提出可优化项，而没有指出需求缺口、正确性风险、测试缺口或明显可维护性风险，不要因为“还能优化”就继续返回 `rerun_execution`。
- 如果没有阻塞项，且没有值得继续 rerun execution 的高价值修订项，就返回 `done`。
